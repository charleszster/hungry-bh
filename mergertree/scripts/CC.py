...
# if one value, assume Benchmark Model, input is z
    if length == 1:
      if input_params[0] > 100:
        z = input_params[0]/299790.		# velocity to redshift
      else:
        z=input_params[0]				# redshift
      H0 = 75							# Hubble constant
      WM = 0.3							# Omega(matter)
      WV = 1.0 - WM - 0.4165/(H0*H0)	# Omega(vacuum) or lambda

# if two values, assume Benchmark Model with given Ho
    elif length == 2:
      z = input_params[0]				# redshift
      H0 = input_params[1]				# Hubble constant
      WM = 0.3							# Omega(matter)
      WV = 1.0 - WM - 0.4165/(H0*H0)	# Omega(vacuum) or lambda
...
# if Univ is Open, use Ho, Wm and set Wv to 0.
      ...
      z = input_params[0]	# redshift
      H0 = input_params[1]	# Hubble constant
      WM = input_params[2]	# Omega(matter)
...
# if Univ is General, use Ho, Wm and given Wv
      ...
      H0 = input_params[1]	# Hubble constant
      WM = input_params[2]	# Omega(matter)
      WV = input_params[3]	# Omega(vacuum) or lambda
...
Options:   -h for this message 
      ...
      if sys.argv[1] == '-v':
        ...
        get_output_params(input_params, verbose=True)
      else:
